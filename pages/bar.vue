  ...
  <div class="min-h-screen bg-gray-50 flex items-center justify-center p-4 sm:p-6">
    <div class="max-w-6xl w-full">
      ...
      <h1 class="text-2xl sm:text-3xl font-bold text-gray-800 text-center mb-6 sm:mb-10">
        Vẽ Biểu Đồ Cột Online - Công Cụ Miễn Phí
      </h1>
      <!-- Đoạn mô tả giới thiệu -->
      <p class="text-gray-600 text-center mb-6">
        Công cụ vẽ biểu đồ cột online miễn phí giúp bạn trực quan hóa dữ liệu nhanh chóng. Nhập dữ liệu, tùy chỉnh nhãn và tải xuống biểu đồ dễ dàng, hỗ trợ mọi thiết bị!
      </p>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-lg p-4 sm:p-6 shadow-sm border border-gray-100">
          <div ref="barChartDiv" class="w-full chart-container"></div>
          <!-- Nội dung bổ sung cho SEO -->
          <div class="mt-6 p-4 bg-gray-100 rounded-lg">
            <h4 class="text-md font-medium mb-2">Biểu đồ cột là gì?</h4>
            <p class="text-sm text-gray-600">
              Biểu đồ cột (bar chart) là một loại biểu đồ dùng để so sánh giá trị giữa các danh mục khác nhau. Đây là công cụ phổ biến trong phân tích dữ liệu, giúp bạn dễ dàng hình dung xu hướng và sự khác biệt.
            </p>
          </div>
        </div>
      </div>
...
useHead({
  title: 'Vẽ Biểu Đồ Cột Online - Công Cụ Miễn Phí',
  meta: [
    { charset: 'utf-8' },
    { name: 'viewport', content: 'width=device-width, initial-scale=1' },
    {
      hid: 'description',
      name: 'description',
      content: 'Công cụ vẽ biểu đồ cột online miễn phí, dễ sử dụng để trực quan hóa dữ liệu. Nhập nhãn và giá trị, tải xuống biểu đồ nhanh chóng trên mọi thiết bị.'
    },
    {
      name: 'keywords',
      content: 'vẽ biểu đồ cột, biểu đồ cột online, công cụ vẽ biểu đồ, bar chart online, trực quan hóa dữ liệu'
    },
...
// Hàm vẽ biểu đồ
const renderChart = () => {
  if (!Plotly || !barChartDiv.value) return
  ...
  Plotly.newPlot(barChartDiv.value, barData, layout, config)
}

// Xử lý resize
const handleResize = () => {
  if (Plotly && barChartDiv.value) {
    Plotly.Plots.resize(barChartDiv.value)
  }
}
...
onMounted(async () => {
  if (process.client) {
    Plotly = await import('plotly.js-dist') // Quay lại phiên bản hiện tại
    renderChart()
...
const downloadChart = () => {
  if (Plotly && barChartDiv.value) {
    Plotly.downloadImage(barChartDiv.value, { 
      format: 'png', 
      filename: 'bieu-do-cot', 
      width: 1000, 
      height: 600 
    })
  }
